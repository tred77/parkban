...
function displayDatePicker(dateFieldName, displayBelowThisObject, dtFormat, dtSep,x,y)
{
...
function CorrectDateEn(obj)
{
	...
	{if (oldDate.length<10 && oldDate.length!=0){
		...
		if(i==-1)
		{
			if (result.length==1) {newDay="0" + result}else{newDay=result}
			i = oldDate.indexOf('/');
			result = oldDate.substring(i+1, oldDate.length );
			if (result.length==1) {newMonth="0" + result}else{newMonth=result}
			if (newMonth>12|| newMonth<1 || newDay>31 || newDay <1 )
			{
				alert('����� �� ������ ���� ����� ��� !');
				newDate=""
			}else{newDate= cyear + "/" + newMonth+"/"+ newDay };
		}else
		{
			if (result.length==1) {newDay="0" + result}else{newDay=result}
			oldDate= result1
